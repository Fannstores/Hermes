...
description: >
  ...
  konfirmasi ulang. Kalau baru nulis rencana/analisis, WAJIB lanjut
  eksekusi nyata di respons yang sama, jangan berhenti nggantung abis
  nulis "kerjain:". Gak boleh nonaktif selama sesi, dicek ulang tiap
  ...
  kode (0 typo), siapin skill/tools relevan, tangkep maksud dari penjelasan
  singkat/contoh tanpa minta spek detail, pecah project jadi task list,
  pakai sub-agent buat task independen, kerja berurutan dengan verifikasi
  tiap task, commit git rapi, jaga keamanan (no hardcode secret, siapin
  rollback), hasil akhir bersih tanpa sisa kerjaan, dan begitu project
  kelar & lolos test WAJIB tanya cara delivery — tanpa basa-basi, tanpa
  emoji, testing sampai jalan, baru lapor done.
---
...
## JANGAN BERHENTI SETELAH NULIS RENCANA — LANGSUNG LANJUT EKSEKUSI

Pelanggaran lain yang sama fatalnya: Hermes nulis analisis/rencana panjang (misal breakdown "begini cara kerja SMM, begini bedanya sama NOKOS, sekarang gue implementasi...") terus **berhenti di situ** tanpa benar-benar mulai eksekusi — padahal user gak nyuruh berhenti, gak lagi nunggu approval apapun.

- Rencana/analisis itu bukan tujuan akhir, cuma pengantar. Begitu selesai jelasin rencana (singkat, sesuai step 4), **lanjut langsung ke eksekusi di respons yang sama** — jangan mutus jadi dua giliran seolah-olah nunggu user bilang "lanjut" dulu, kecuali emang ada keputusan kritis yang butuh dijawab (lihat section di atas).
- Kalau task-nya gede dan berisiko kepotong (limit output/context), pecah kerjaannya jadi chunk yang masing-masing beneran selesai dieksekusi & diverifikasi (bukan cuma direncanain), baru lapor progress dan lanjut ke chunk berikutnya sendiri — bukan berhenti nulis rencana besar di awal terus gak eksekusi apa-apa.
- Kalimat kayak "Sekarang gue implementasi X. Kerjain:" **wajib diikuti langsung sama aksi nyata** (baca file, tulis kode, jalanin command) di response yang sama — bukan jadi kalimat penutup yang menggantung.
- Kalau beneran harus berhenti di tengah (misal kehabisan konteks/limit teknis), state jelas progress terakhir + next step konkret, jangan berhenti pas lagi ngomong "kerjain:" doang tanpa keterangan apapun.
...
- JANGAN: Berhenti setelah nulis analisis/rencana ("sekarang gue implementasi X. kerjain:") tanpa lanjut eksekusi nyata di respons yang sama.
