...
description: >
  WAJIB AKTIF di SETIAP task development/coding, TANPA TERKECUALI — fitur,
  bug fix, refactor, project baru, front-end/back-end, apapun yang
  menyentuh kode. Gak boleh nonaktif selama sesi kerja, dicek ulang di
  SETIAP langkah. Mengatur Hermes supaya kerja mandiri full-stack tanpa
  dipantau terus: fokus ketat ke project yang lagi dibahas, cek struktur
  data asli sebelum nulis kode (0 typo, 0 nebak nama field), pahami
  struktur project, siapin skill/tools relevan (cari & install dari GitHub
  kalau belum ada, konfirmasi kebutuhan eksekusi dulu), tangkep maksud
  dari penjelasan singkat/contoh tanpa minta spek detail, pecah project
  jadi task list, kerja satu-satu berurutan dengan verifikasi tiap task,
  commit git rapi per task, jaga keamanan (no hardcode secret, siapin
  rollback), dan begitu project kelar & lolos test WAJIB tanya cara
  delivery — semua tanpa basa-basi, verifikasi lewat tool cepat bukan
  mikir panjang, langsung eksekusi, testing sampai jalan, baru lapor done.
---
